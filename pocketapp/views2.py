...
import urllib, urllib2, cookielib, socket, hashlib, time, re, json
		...
		if params.has_key('echostr'):
			...
			if request.raw_post_data:
				...
				if event == 'subscribe' or content == 'Hello2BizUser':
					msg = reply_text('欢迎来到“我的Pocket”, 回复“a”绑定Pocket账号。由于国内访问Pocket速度较慢，如无回应请重新发送。\r\n回复“h”或“?”获取帮助。')
					...
					item = reply['item'] % ('点击绑定Pocket账号', '', POCKET['pic_authorization'], authorization_uri)
					item += reply['item'] % ('如果你还没有Pocket账号，点击这里注册', '', POCKET['pic_signup'], POCKET['signup_uri'])
					msg = reply['news'] % (toUserName, fromUserName, postTime, '2', item)
					...
					if user_item.count() == 0 or user_item2.count() == 0:
						msg = reply_text('使用帮助：\r\n回复“a”绑定Pocket账号。由于国内访问Pocket速度较慢，如无回应请重新发送。\r\n\r\n有问题或者建议欢迎留言或者发送邮件到devange@live.com，感谢你的支持！')
					else:
					...
					user_item2 = User2.objects.order_by('id').filter(wechat_user=uid)
					if user_item.count() == 0 or user_item2.count() == 0:
						msg = reply_text('你还没有绑定Pocket账号。\r\n点击菜单绑定Pocket账号。由于国内访问Pocket速度较慢，如无回应请重新操作。')
					else:
						count = 5
						...
						try:
							...
							msg = reply_text('非常抱歉，新增的获取“最近待读”功能需要您重新授权Pocket账号才能使用。\r\n请点击菜单重新绑定账号。')
							response = 'timeout'
				else:
					user_item = User.objects.order_by('id').filter(wechat_user=uid)
					user_item2 = User2.objects.order_by('id').filter(wechat_user=uid)
					if user_item.count() == 0 or user_item2.count() == 0:
						msg = reply_text('回复“a”绑定Pocket账号。由于国内访问Pocket速度较慢，如无回应请重新发送。\r\n回复“h”或“?”获取帮助。')
						...
						if msgtype == 'link' or content[0:7] == 'http://' or content[0:8] == 'https://':
							...
							try:
								...
								try:
									...
									msg = reply_text('保存失败或超时，请确认链接有效，或尝试重新绑定账号后重试。')
									response = 'timeout'
...
def authorizationFinished(request):
	...
	if auth_result == 0:
		title = '绑定失败'
		msg = '账号绑定失败，请返回微信回复“a”重试。'
	elif auth_result == 1:
		title = '绑定成功'
		msg = pocket_user + '账号绑定成功。<br>返回微信给我发送链接即可保存到Pocket以便稍后阅读。'
	elif auth_result == 2:
		title = '账号已绑定'
		msg = '你已经绑定过Pocket账号<br>返回微信给我发送链接即可保存到Pocket以便稍后阅读，或者回复“a”重新绑定。'
